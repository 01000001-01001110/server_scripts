...
docker run -d -it -e EULA=TRUE --name $contname -e SERVER_NAME=$servername -e GAMEMODE=$gamemode -e ONLINE_MODE=$online -e level_seed=$seed -e difficulty=normal -e spawn-npcs=$npc -e spawn-monsters=$monster -e spawn-animals=$animal -p 19132:19133/udp itzg/minecraft-bedrock-server --restart always
